...
re_scale = 2.0
ABCDs = np.array([[ 1.                 ,  0.                 ,  0.                 ,  1.154*re_scale     , -1.154*re_scale],
                  [ 1.081              ,  1.                 , -0.0701             ,  0.                 ,  0.   ],
                  [ 0.                 ,  0.827              ,  1.                 ,  0.                 ,  0.   ],
...
Cbank = Cyinput+Cyint1+Cyint2+Cyint3+Cflash

flash_scale = 1/(Cyinput/Cbank)
...
#    dmismatch = 0.01/np.sqrt(Cin*1e15/(nlev-1)) ## Regular MOM sigma
    ...
    if amp_index == 2:
        ...
        pl.plot([0.0005, 0.5/(over*OSR)], [-100, -100], 'k', linewidth=10, label='Signal Band')
        ds.figureMagic([0.0005, 0.5], None, None, [-100, 20], 10, None, (16, 6), 'Output Spectrum')
...
ds.figureMagic([-5, 5], 1, None, [40, 78], 2, None, (10, 6), 'OUT Dynamic Range')
...
## Calculation for a polyphase extrapolation (z^2)
...
z2,p2,k2 = sp.signal.tf2zpk(new_num, new_den)
z2 = np.append(z2, 1.0)
z2 = np.append(z2, 1.0)
ntf2 = tuple([z2, p2, k2])
