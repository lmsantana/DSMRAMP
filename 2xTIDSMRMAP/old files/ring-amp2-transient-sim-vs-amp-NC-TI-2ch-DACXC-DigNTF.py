...
#    u = (amp[amp_index] * (np.sin(2*np.pi* fin/N *np.arange(N))))*sin2di_dBFS + input_noise
    ...
    for i in range(N):
        ...
        if i%2 == 0:
#            y01int = np.real(np.dot(C, x01) + np.dot(D1, u[:,i])) -(veq1)+2*0.945*(veq2)
#            y01int = np.real(np.dot(C, x01) + np.dot(D1, u[:,i])) +veq2z1 -3*(veq1) + 3*(veq2)
            y01int = np.real(np.dot(C, x01) + np.dot(D1, u[:,i])) +veq2z1 -2.95*(veq1) + 2.95*(veq2)
#            y01int = np.real(np.dot(C, x01) + np.dot(D1, u[:,i])) -veq1z1 +4*veq2z1 -6*(veq1) + 4*(veq2)
            ...
            vbm11 = ds_quantize((y01int-vb51-vb41-vb31-vb21-vb11-vb01)     ,2)*0.5
            vbm21 = ds_quantize((y01int-vb51-vb41-vb31-vb21-vb11-vb01-vbm11),2)*0.25
            vbm31 = ds_quantize((y01int-vb51-vb41-vb31-vb21-vb11-vb01-vbm11-vbm21),2)*0.125
            vbm41 = ds_quantize((y01int-vb51-vb41-vb31-vb21-vb11-vb01-vbm11-vbm21-vbm31),2)*0.0625
...
#                sys.exit('fault calculation')
        ...
        if i%2 == 1:
#            y02int = np.real(np.dot(C, x02) + np.dot(D1, u[:,i])) -(veq2) + 2*0.945*(veq1)
#            y02int = np.real(np.dot(C, x02) + np.dot(D1, u[:,i])) +veq1z1 - 3*(veq2) + 3*(veq1)
            y02int = np.real(np.dot(C, x02) + np.dot(D1, u[:,i])) +veq1z1 - 2.95*(veq2) + 2.95*(veq1)
#            y02int = np.real(np.dot(C, x02) + np.dot(D1, u[:,i])) -veq2z1 +4*veq1z1 -6*(veq2) + 4*(veq1)
            ...
            vbm12 = ds_quantize((y02int-vb52-vb42-vb32-vb22-vb12-vb02)     ,2)*0.5
            vbm22 = ds_quantize((y02int-vb52-vb42-vb32-vb22-vb12-vb02-vbm12),2)*0.25
            vbm32 = ds_quantize((y02int-vb52-vb42-vb32-vb22-vb12-vb02-vbm12-vbm22),2)*0.125
            vbm42 = ds_quantize((y02int-vb52-vb42-vb32-vb22-vb12-vb02-vbm12-vbm22-vbm32),2)*0.0625
