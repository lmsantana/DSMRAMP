...
for amp_index in range(np.size(amp)-10):
...
#        y0 = np.real(np.dot(C, x0) + np.dot(D1, u[:,i]))
        y0 = np.real(np.dot(C, x0) + np.dot(D1, u[:,i])) + B[0,1]*C[0,0]*vdac2int0
        ...
        msb0 = np.floor(vbit[:,i]/(nlev/2))
        msb1 = np.floor((vbit[:,i]-msb0*nlev/2)/(nlev/4))
        msb2 = np.floor((vbit[:,i]-msb0*nlev/2 -msb1*nlev/4)/(nlev/8))
        
        vbitdac1[:,i] = (nlev/8)*(4*msb0 + 2*msb1 + msb2)
        # vdac1[:,i] = vbitdac1[:,i]*2 -24  # For a 2b+3b partition
        vdac1[:,i] = vbitdac1[:,i]*2 -28    # For a 3b+2b partition
        vdac2[:,i] = v[:,i] - vdac1[:,i]
        
        ### Full DAC feedback at input of INT1
#        x0 = np.dot(A, x0) + np.dot(B, np.concatenate((u[:,i], v[:,i])))
        
        ## MSB DAC feedback at input of INT1
        x0 = np.dot(A, x0) + np.dot(B, np.concatenate((u[:,i], vdac1[:,i]))) + F*vdac2int0
        vdac2int0 = 0.99009901*vdac2int0 + vdac2[:,i] 
        vdac2int[:,i] = vdac2int0
...
ds.figureMagic([-80, 5], 5, None, [-10,90], 5, None, (10, 6), 'OUT Dynamic Range')
