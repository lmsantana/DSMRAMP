...
np.random.seed(1)
##########################
peak_sndr = DCgain*0.0
msa = DCgain*0.0
for dcgain_idx in range(np.size(DCgain)):
    Cin = 125e-15*5
    ...
    if finite_dc_gain == True:    
        av = ds.undbv(DCgain[dcgain_idx]) # 60db
        a,g,b,c = ds.mapABCD(ABCDs, form)
...
#        abcd = ds.stuffABCD(a,g,b,c,form)
    ...
    #            u[i] = u[i] + noise_enb*pl.sqrt(sigma2_sw_in)*(np.random.randn(1)[0])*sin2se_dBFS 
        ...
        for i in range(N):
            ...
            if i%2 == 0:
                y01int = np.real(np.dot(C, x01) + np.dot(D1, u[:,i]+ noise_enb*pl.sqrt(sigma2_sw_in)*(np.random.randn(1)[0])*sin2se_dBFS))+opt_g2*2*(vbm12+vbm22+vbm32+vbm42) ## mid conversion coupling signal
                y01int = y01int*y_att
                y1[:,i] = y01int
                yz1 = y01int
    ...
    #                sys.exit('fault calculation')
                ...
                x01 = np.dot(A, x01) + np.dot(B, np.concatenate((u[:,i]+ noise_enb*pl.sqrt(sigma2_sw_in)*(np.random.randn(1)[0])*sin2se_dBFS , v1[:,i])))
            ...
            if i%2 == 1:
                y02int = np.real(np.dot(C, x02) + np.dot(D1, u[:,i]+ noise_enb*pl.sqrt(sigma2_sw_in)*(np.random.randn(1)[0])*sin2se_dBFS ))+opt_g2*2*(vbm11+vbm21+vbm31+vbm41)  ## mid conversion coupling signal
                y02int = y02int*y_att
                y2[:,i] = y02int
                yz2 = y02int
    ...
    #                sys.exit('fault calculation')
                ...
                ### Full DAC feedback at input of INT1
                x02 = np.dot(A, x02) + np.dot(B, np.concatenate((u[:,i]+ noise_enb*pl.sqrt(sigma2_sw_in)*(np.random.randn(1)[0])*sin2se_dBFS , v2[:,i])))
...
#    pl.legend(loc=4)
#    pl.plot()
    peak_sndr[dcgain_idx] = snr_amp.max()
    msa[dcgain_idx] = ds.dbv(amp[np.argmax(snr_amp)])
    print(dcgain_idx)

pl.figure(figsize=(10,4))
pl.plot(DCgain,peak_sndr)
ds.figureMagic([5, 50], 5, None, [60,100], 2, None, (10, 6), 'OUT Dynamic Range')
...
pl.figure(figsize=(10,4))
pl.plot(DCgain,msa)
ds.figureMagic([5, 50], 5, None, [0,4], 0.5, None, (10, 6), 'OUT Dynamic Range')
pl.xlabel('DC gain (dB)')
pl.ylabel('Peak SNDR (dB)')
pl.plot()
