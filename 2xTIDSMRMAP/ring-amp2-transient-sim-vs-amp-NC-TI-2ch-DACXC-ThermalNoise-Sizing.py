...
for mc_index in range(mc_points):
    ...
    input_noise = noise_enb*pl.sqrt(sigma2_sw_in)*(np.random.randn(N))*sin2se_dBFS*0
...
#                      [ 0.642     ,  0.246    ,  0.103    ,  1.       ,  0.   ]])
    ...
    state_samp_cap = np.array([Cin_int2 +Cy1 +Cint1*Cin_int1/(Cint1+Cin_int1), 
                               Cin_int3 +Cy2 +Cint2*(Cin_int2+Cg2)/(Cint2+Cin_int2+Cg2), 
                               Cy3+Cg2+Cint3*Cin_int3/(Cint3+Cin_int3)])*1e-15
    ysamp_cap = (Cq + Cint_sum*(Cy1+Cy2+Cy3+Cyu)/(Cint_sum+Cy1+Cy2+Cy3+Cyu))*1e-15
    
    state_samp_noise = noise_enb*pl.sqrt(4*4e-21/state_samp_cap)*sin2se_dBFS*np.array([0,0,0])
    y_samp_noise = noise_enb*pl.sqrt(4*4e-21/ysamp_cap)*sin2se_dBFS*1
    ...
    #    u = (amp[amp_index] * (np.sin(2*np.pi* fin/N *np.arange(N))))*sin2di_dBFS + input_noise
        ...
        for i in range(N):
            if i%2 != 0:
                u[i] = u[i] + noise_enb*pl.sqrt(sigma2_sw_in)*(np.random.randn(1)[0])*sin2se_dBFS*0
            else:
                u[i] = u[i] + noise_enb*pl.sqrt(sigma2_sw_in)*(np.random.randn(1)[0])*sin2se_dBFS*0
        ...
        A2 = ABCDs[:order, :order]
        B2 = ABCDs[:order, order:order+nu+nq]
        C2 = ABCDs[order:order+nq, :order]
        D12 = ABCDs[order:order+nq, order:order+nu] ## assuming no direct feedback from V to Y
        ...
        for i in range(N):
            ...
            if i%2 == 0:
                y01int = np.real(np.dot(C1, x01) + np.dot(D11, u[:,i])) - \
                         opt_g2*(round(vbm11[0]/0.5)*0.5 + \
                                 round(vbm21[0]/0.25)*0.25+ \
                                 round(vbm31[0]/0.125)*0.125 + \
                                 round(vbm41[0]/0.0625)*0.0625) +\
                         y_samp_noise*np.random.randn(1)
                y1[:,i] = y01int
                yz1 = y01int
                ...
                y01int = y01int-vb11
                y01int = y01int + opt_g2*(round(vbm12[0]/0.5) + \
                                          round(vbm22[0]/0.25)*0.5 + \
                                          round(vbm32[0]/0.125)*0.25 + \
                                          round(vbm42[0]/0.0625)*0.125)
                ...
                x01 = np.dot(A1, x01+state_samp_noise*np.random.randn(3)) + np.dot(B1, np.concatenate((u[:,i], v1[:,i])))
            ...
            if i%2 == 1:
                y02int = np.real(np.dot(C2, x02) + np.dot(D12, u[:,i])) - \
                         opt_g2*(round(vbm12[0]/0.5)*0.5+ \
                                 round(vbm22[0]/0.25)*0.25+ \
                                 round(vbm32[0]/0.125)*0.125 + \
                                 round(vbm42[0]/0.0625)*0.0625)+\
                         y_samp_noise*np.random.randn(1)
                ...
                y02int = y02int-vb12
                y02int = y02int + opt_g2*(round(vbm11[0]/0.5) + \
                                          round(vbm21[0]/0.25)*0.5 + \
                                          round(vbm31[0]/0.125)*0.25 + \
                                          round(vbm41[0]/0.0625)*0.125)
...
#                vdac1 = vb51+vb41+vb31+vb21+vb11+vb01                    
                ...
                ### Full DAC feedback at input of INT1
                x02 = np.dot(A2, x02+state_samp_noise*np.random.randn(3)) + np.dot(B2, np.concatenate((u[:,i], v2[:,i])))
...
power_est_LP = 7.3e12*(np.sum(state_samp_cap)+ysamp_cap)
print('Estimated LP power:')
print(power_est_LP)

print('State sampling load:')
print(state_samp_cap*1e15)

print('Y sampling load:')
print(ysamp_cap*1e15)
