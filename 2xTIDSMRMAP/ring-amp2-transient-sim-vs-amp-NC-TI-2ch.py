...
#            sum1 = np.real(np.dot(C, x01) + np.dot(D1, u[:,i])) + (-sum1 + vz1 +2*vz2p) + 2*sum2 -4*vz1 
#            y01 = sum1 - 2*vz2
...
#            sum2 = np.real(np.dot(C, x02) + np.dot(D1, u[:,i])) + (-sum2 + vz2 +2*vz1p) + 2*sum1 -4*vz2 
#            y02 = sum2 - 2*vz1
...
pl.text(-40, 75,'Peak SQNR = %2.1f dB @ %2.1f dbV' %(snr_amp.max(), ds.dbv(amp[np.argmax(snr_amp)])))
