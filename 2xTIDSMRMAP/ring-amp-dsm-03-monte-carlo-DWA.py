...
if noise_in_type == 1: ## Split Input and DAC CAP structure
    ...
    sigma2_dac_in = 0
    sigma2_sw_in = 1 * 4e-21 / Cin          # 0.38 factor from the integrated sinc^2

amp_psd = 1e-5 ## V/sqrt(Hz)
sigma2_amp = np.array([(amp_psd)**2 , (amp_psd)**2, (amp_psd)**2])

u = (amp * np.sin(2*np.pi* fin/N *np.arange(N))) + noise_enb*pl.sqrt(sigma2_sw_in)*(np.random.randn(N))*sin2se_dBFS
...
snr_mis = np.ones(1000)
snr_DWA = np.ones(1000)
...
pl.subplot(121)
pl.hist(snr_mis, bins=100)
pl.title('Thermometer code')

pl.subplot(122)
pl.hist(snr_DWA, bins=100)
pl.title('DWA code')
pl.show()

snr_sorted = np.sort(snr_mis)
snr_DWA_sorted = np.sort(snr_DWA)
#pl.plot(np.linspace(1000,1,num=1000)/1000, snr_sorted)
#pl.show()
print("\n 3-sigma, SNR = %2.2f dB" %(snr_sorted[1]))
print("\n 3-sigma, SNR = %2.2f dB" %(snr_DWA_sorted[1]))

#print("\n 2.5-sigma, SNR = %2.2f dB" %(snr_sorted[22]))
#print("\n 2-sigma, SNR = %2.2f dB" %(snr_sorted[136]))
#print("\n Average, SNR = %2.2f dB" %(np.average(snr_sorted)))
#print("\n Median, SNR = %2.2f dB" %(np.median(snr_sorted)))
...
### Mismatch
pl.semilogx(f, ds.dbv(specDWA[:int(N/2.) +1]), '--r', label='Simulated Specturm with mismatch')
snrm = ds.calculateSNR(specDWA[2:fb+1], fin-2)
