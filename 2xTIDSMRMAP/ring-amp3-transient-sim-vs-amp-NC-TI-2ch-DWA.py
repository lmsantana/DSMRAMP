...
B = ABCDs[:order, order:order+2]
ndac = nlev-1
point1 = 0
cal_red = 10
v_coef1 = np.ones(ndac)*(B[0,1]/ndac)*(1+dmismatch*np.random.randn(ndac))
if msb_dac_cal == True:
    v_coef1[0:int((ndac+1)/2)] = np.ones(int((ndac+1)/2))*(B[0,1]/ndac)*(1+(dmismatch/cal_red)*np.random.randn(int((ndac+1)/2)))
if msb_1_dac_cal == True:
    v_coef1[int((ndac+1)/2):int((ndac+1)/2)+int((ndac+1)/4)] = np.ones(int((ndac+1)/4))*(B[0,1]/ndac)*(1+(dmismatch/cal_red)*np.random.randn(int((ndac+1)/4)))
v_coef_conc1 = np.concatenate((v_coef1,v_coef1))

point2 = 0
dmismatch = 0.1/100.0
v_coef2 = np.ones(ndac)*(B[0,1]/ndac)*(1+dmismatch*np.random.randn(ndac))
if msb_dac_cal == True:
    v_coef2[0:int((ndac+1)/2)] = np.ones(int((ndac+1)/2))*(B[0,1]/ndac)*(1+(dmismatch/cal_red)*np.random.randn(int((ndac+1)/2)))
if msb_1_dac_cal == True:
    v_coef2[int((ndac+1)/2):int((ndac+1)/2)+int((ndac+1)/4)] = np.ones(int((ndac+1)/4))*(B[0,1]/ndac)*(1+(dmismatch/cal_red)*np.random.randn(int((ndac+1)/4)))
v_coef_conc2 = np.concatenate((v_coef2,v_coef2))
...
#    u = (amp[amp_index] * (np.sin(2*np.pi* fin/N *np.arange(N))))*sin2di_dBV + input_noise
    ...
    for i in range(N):
        ...
        if i%2 == 0:
            y01 = np.real(np.dot(C, x01) + np.dot(D1, u[:,i])) + (vz1-yz1)  - 2*(vz2-yz2)
...
#            y01 = np.real(np.dot(C, x01) + np.dot(D1, u[:,i]))
            ...
            ### DAC mismatch and DWA implementations
            nelements = int((ndac + v[:,i])/2)
            p_coef = np.sum(v_coef_conc1[point1           : point1+nelements])
            n_coef = np.sum(v_coef_conc1[point1+nelements : point1+nelements + (ndac-nelements)])
            BxV_input_sum = (n_coef*(-ndac) + p_coef*ndac)/B[0,1]
            in_vector = np.concatenate((u[:,i], np.array([BxV_input_sum])))
            
            ## update of pointer value
            if dwa_on == True:
                if point1+nelements >= nlev-1:
                    point1 = nelements-(nlev-1-point1)
                else:
                    if point1+nelements == nlev-1:
                        point1 = 0
                    else:
                        point1 = point1+nelements
            ### Full DAC feedback at input of INT1
#            x01 = np.dot(A, x01) + np.dot(B, np.concatenate((u[:,i], v1[:,i])))
            x01 = np.dot(A, x01) + np.dot(B, in_vector)
        ...
        if i%2 == 1:
            y02 = np.real(np.dot(C, x02) + np.dot(D1, u[:,i])) + (vz2-yz2) - 2*(vz1-yz1)
...
#            y02 = np.real(np.dot(C, x02) + np.dot(D1, u[:,i]))
            ...
            ### DAC mismatch and DWA implementations
            nelements = int((ndac + v[:,i])/2)
            p_coef = np.sum(v_coef_conc2[point2           : point2+nelements])
            n_coef = np.sum(v_coef_conc2[point2+nelements : point2+nelements + (ndac-nelements)])
            BxV_input_sum = (n_coef*(-ndac) + p_coef*ndac)/B[0,1]
            in_vector = np.concatenate((u[:,i], np.array([BxV_input_sum])))
            
            ## update of pointer value
            if dwa_on == True:
                if point2+nelements >= nlev-1:
                    point2 = nelements-(nlev-1-point2)
                else:
                    if point2+nelements == nlev-1:
                        point2 = 0
                    else:
                        point2 = point2+nelements
            ### Full DAC feedback at input of INT1
#            x02 = np.dot(A, x02) + np.dot(B, np.concatenate((u[:,i], v2[:,i])))
            x02 = np.dot(A, x02) + np.dot(B, in_vector)
