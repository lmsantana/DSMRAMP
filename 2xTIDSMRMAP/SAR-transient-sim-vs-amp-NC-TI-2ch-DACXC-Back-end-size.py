...
snr_bits = np.zeros((np.size(amp), 7))
spec_bit = np.zeros((int(N/2)+1 , 7))

for n_bit_back in range(0,7):
    ...
    #    u = (amp[amp_index] * (np.sin(2*np.pi* fin/N *np.arange(N))))*sin2di_dBFS + input_noise
        ...
        for i in range(N):
            ...
            if i%2 == 0:
                ...
                if n_bit_back == 1:
                    ...
                    veq1 = vbm11 + vbm21 + vbm31 
                elif n_bit_back == 4:
                    veq1 = vbm11 + vbm21 + vbm31 + vbm41 
                elif n_bit_back == 5:
                    veq1 = vbm11 + vbm21 + vbm31 + vbm41 + vbm51
                else:
            ...
            if i%2 == 1:
                ...
                if n_bit_back == 1:
                    ...
                    veq2 = vbm12 + vbm22 + vbm32 
                elif n_bit_back == 4:
                    veq2 = vbm12 + vbm22 + vbm32 + vbm42 
                elif n_bit_back == 5:
                    veq2 = vbm12 + vbm22 + vbm32 + vbm42 + vbm52
                else:
        ...
        print(amp_index)
        if amp_index == 5:
            spec_bit[:, n_bit_back] = ds.dbv(spec[:int(N/2.) +1])
        #     pl.figure()
    ...
    snr_bits[:, n_bit_back] = snr_amp

pl.rcParams['lines.linewidth'] = 3
pl.figure(figsize=(10,10))
for n_bit_back in range(7):
    if n_bit_back == 0:
        pl.plot(ds.dbv(amp) - 5, snr_bits[:,n_bit_back], label='Analog')
    else:
        pl.plot(ds.dbv(amp) - 5, snr_bits[:,n_bit_back], label='Nbits = %d' %n_bit_back)
...
snr_max = np.zeros(7)
snr_bits_max = np.max(snr_bits, axis=0)
snr_max[0:6] = snr_bits_max[1:7]
snr_max[6] = snr_bits_max[0]
pl.plot(range(1,8), snr_max, 'o-')
...
x1, x2 = ax.get_xlim()
ax.xaxis.set_ticks(np.arange(x1, x2, dx))
ax.xaxis.set_ticklabels(['1', '2', '3', '4', '5', '6', 'Analog'])
y1, y2 = ax.get_ylim()
ax.yaxis.set_ticks(np.arange(y1, y2, dy))

pl.title('Peak SQNR across \n back-end quantizer size')
pl.xlabel('Number of bits')
