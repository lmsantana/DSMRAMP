...
#ntf = ds.clans(order = order, OSR = OSR, Q = nlev/2, opt=1)
#ntf = ds.synthesizeNTF(order=order, osr=OSR, opt=1, H_inf=3.0)
...
## Scalling
ABCDs, umax, _ = ds.scaleABCD(ABCD, nlev=nlev, xlim=np.array([0.35*nlev/2,0.35*nlev/4,0.35*nlev/8]), N_sim=10000)
maxima = np.zeros((order, len(u)))
ymax = np.zeros((1,len(u)))
for i in range(len(u)):
    u_sim = u[i]
...
ds.DocumentNTF(ntf, OSR)
ds.plotPZ(ntf, 'b', 6, showlist=True)
